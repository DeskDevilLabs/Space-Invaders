...
class Player:
    ...
    def update(self, can_move=True):
        ...
        self.is_invincible = keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT]
...
class Game:
    ...
    # Level configurations
        self.level_configs = {
            1: {
                'rows': 4, 'cols': 8, 'types': [1, 1], 'speed': 1, 'shoot_chance': 0.015,
                'name': 'LEVEL 1: Basic Formation'
            },
            2: {
                'rows': 5, 'cols': 9, 'types': [1, 1, 2], 'speed': 2, 'shoot_chance': 0.018,
                'name': 'LEVEL 2: Mixed Forces'
            },
            3: {
                'rows': 5, 'cols': 10, 'types': [1, 2, 2, 3], 'speed': 3, 'shoot_chance': 0.02,
            ...
            5: {
                'rows': 6, 'cols': 12, 'types': [3, 3, 4, 4, 5, 5], 'speed': 4, 'shoot_chance': 0.029,
                'name': 'LEVEL 5: BOSS WAVE'
            },
            6: {
                'rows': 7, 'cols': 12, 'types': [3, 3, 4, 4, 5, 5], 'speed': 4, 'shoot_chance': 0.031,
                'name': 'LEVEL 6: ARMADA APPROACHES'
            },
            7: {
                'rows': 7, 'cols': 12, 'types': [3, 4, 4, 4, 5, 5], 'speed': 4, 'shoot_chance': 0.035,
                'name': 'LEVEL 7: DANGER ZONE'
            },
            8: {
                'rows': 8, 'cols': 13, 'types': [4, 4, 4, 4, 5, 5], 'speed': 5, 'shoot_chance': 0.039,
                'name': 'LEVEL 8: ARMAGEDDON'
            },
            9: {
                'rows': 8, 'cols': 13, 'types': [4, 4, 4, 5, 5, 5], 'speed': 5, 'shoot_chance': 0.044,
                'name': 'LEVEL 9: FINAL DEFENSE'
            },
            10: {
                'rows': 9, 'cols': 14, 'types': [4, 5, 5, 5, 5, 5], 'speed': 5, 'shoot_chance': 0.049,
    ...
    def draw_confirmation_dialog(self):
        ...
        screen.blit(confirm_text, confirm_text.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2)))
    ...
    def draw_exit_confirmation(self):
        ...
        if hasattr(self, 'exit_confirmed') and self.exit_confirmed:
            ...
            current_time = pygame.time.get_ticks()
            if current_time - self.exit_time >= 3000:  # 3 seconds
                return "exit"
            ...
            # Display the thank you messages with better spacing
            message_font = pygame.font.Font(None, 36)
            message_large_font = pygame.font.Font(None, 48)  # Slightly larger font for first message
            
            message_text = message_large_font.render("Thanks for playing Space Invaders!", True, YELLOW)
            message_text2 = message_font.render("May the Force be with you!", True, CYAN)
            
            # Calculate vertical positions with gap
            first_message_y = SCREEN_HEIGHT // 2 - 50
            second_message_y = SCREEN_HEIGHT // 2 + 20  # 70px gap between messages
            
            screen.blit(message_text, message_text.get_rect(center=(SCREEN_WIDTH//2, first_message_y)))
            screen.blit(message_text2, message_text2.get_rect(center=(SCREEN_WIDTH//2, second_message_y)))
    ...
    def draw(self, screen):
        ...
        if not (self.show_exit_confirmation and hasattr(self, 'exit_confirmed') and self.exit_confirmed):
            if self.show_leaderboard:
                self.draw_leaderboard()
            ...
            if self.show_level_text:
                ...
                if self.won:
                    game_over_text = big_font.render('CONGRATULATIONS ! You Won the Game ', True, GREEN)
                    subtitle_text = font.render('You have defeated all invader waves!', True, YELLOW)
                    self.show_level_text = False

                    menu_text = font.render('Press SPACE/RETURN to return to Main Menu', True, WHITE)
                    quit_text = font.render('Press ESC to Exit Game', True, WHITE)
                ...
                if not self.score_submitted and self.leaderboard_manager.is_high_score(self.score):
                    high_score_text = font.render('NEW HIGH SCORE!', True, YELLOW)
                    screen.blit(high_score_text, high_score_text.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2 - 60)))
...
def main():
    ...
    while running:
        ...
        if hasattr(game, 'exit_confirmed') and game.exit_confirmed:
            ...
            # Add a delay before showing credits to allow player to see victory message
